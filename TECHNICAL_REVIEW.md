...
Version 0.8.0 centralizes application metadata, the approved public author contact, prototype status, disclaimer text, build identifier, and validation reference date. The Streamlit interface adds an About MOBRA introduction, workflow navigation, contextual interpretation boxes, assessment metadata, separate Refresh View and confirmed Reset Assessment controls, printable ORL and hazard templates, a combined field-assessment workbook, optional privacy-conscious email backup, a derived-output ZIP fallback, manuscript status handling, and a Resources and Contact area.
