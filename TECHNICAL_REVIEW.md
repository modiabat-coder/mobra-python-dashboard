...
## UI and operational-tools review

Version 0.9.0 centralizes application metadata, the approved public author contact, prototype status, disclaimer text, build identifier, validation reference date, repository URL, deployed URL, manuscript checksum, and branding version. The Streamlit interface adds an original MOBRA identity, workflow navigation, popover help with expander fallback, once-only notifications, assessment metadata, separate Refresh View and confirmed Reset Assessment controls, printable ORL and hazard templates, a combined field-assessment workbook, optional privacy-conscious email backup, a branded derived-output ZIP fallback, the author-approved manuscript card, educational poster cards, and a Resources and Contact area.

Template builders read the current requirements and hazard schemas. They do not infer assessor-selected Likelihood or Consequence values, mutate sample data, or alter BRI, risk, acceptance, governance, mapping, validation, or decision logic. The dependency-free PDF builder emits repeated-header paper forms and includes the disclaimer; Excel builders use stable sheet names and instructions.

The normative-resource manifest distinguishes free official downloads, official webpages, licensed standards, normative guidance, implementation guidance, advisory best practice, and supporting scientific literature. ISO resources never expose a bundled local PDF and are marked **Do not redistribute**. Official source/download links are centralized in `config/normative_resources.json`; supporting literature metadata is in `config/supporting_literature.json`. The catalogue exports metadata and links only. The application contains a non-endorsement statement and does not imply WHO, ISO, CDC, or NIH endorsement.

Email backup remains opt-in. SMTP credentials are read only from environment variables or Streamlit Secrets, never displayed or committed. Sending requires a valid recipient, assessment name, user consent, institutional authorization, and confirmation that sensitive data are permitted; source uploads are excluded by default. When SMTP is absent, the application remains fully usable and exposes downloadable backups.

The visual identity is original MOBRA artwork documented in `docs/VISUAL_IDENTITY.md`; educational media are registered in `config/educational_media.json` and supplied as SVG, PNG, and PDF. `docs/LIVE_APPLICATION_QA.md` records the live review against the deployed baseline and the local checklist for this branch. Protected risk calculations, demonstration records, governance, mapping, validation, BRI, and deployment-decision rules remain unchanged.
